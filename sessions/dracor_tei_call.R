...
if (!require("jsonlite","httr","purrr","xml2")) 
  install.packages("jsonlite","httr","purrr","xml2")
library(jsonlite)
library(httr)
library(purrr)
library(xml2)
...
api_df<-jsonlite::fromJSON(content(api_call,"text"),simplifyDataFrame = T)
plays<-api_df$dramas
#################
#12216.ascension
#sample GET: 
...
tei_url_full<-paste(play_url,map_chr(get.url,paste),"tei",sep="/")
